...
// L, R, and O are n x m matrices of field elements and **s** is a vector of G1, G2, or G12 points
...
contract HW6 {
    ...
    /*
     L,R,O: n x m matrices 
     s: m x 1 (column vector)
     Ls: n x 1 (column vector)
    */
    function verify(uint256[] calldata L, uint256[] calldata R, uint256[] calldata O, uint256[] calldata s, uint256 n, uint256 m) external view returns(bool) {
        require(L.length == n * m, "Invalid matrix dimension");
        require(s.length == m, "Invalid vector dimension");

        // Ls, RS, Os
        uint256[] memory Ls = matmul2(L, s, n, m);
    ...
    /*
     L: n x m matrices 
     s: m x 1 (column vector)
     return Ls: n x 1 (column vector)
    */
    function matmul2(uint256[] calldata L, uint256[] calldata s, uint256 n, uint256 m) internal pure returns (uint256[] memory) {
        require(L.length == n * m, "Invalid matrix dimension");
        require(s.length == m, "Invalid vector dimension");
